...
class EventDetailsScreen extends StatefulWidget {
  ...
  final String orgId;
  const EventDetailsScreen({super.key, required this.eventId, required this.orgId});
                    ...
                    return StreamBuilder<UserModel>(
                        ...
                        builder: (context, user) {
                          return StreamBuilder<bool>(
                              stream: FireServices.instance.isUserJoinedEvent(orgId: widget.orgId, eventId: widget.eventId, userId: userId.data!),
                                ...
                                return ElevatedButton.icon(
                                    ...
                                    onPressed: () async {
                                      if ((isUserJoined.hasData && isUserJoined.data!)) {
                                        await Utils.leaveConfirmationDialog(context, orgId: widget.orgId, eventId: widget.eventId, userId: userId.data!);
                                      } else {
                                    ...
                                    icon: (isUserJoined.connectionState == ConnectionState.waiting)
                                        ? SizedBox()
                                        : Icon(
                                            (isUserJoined.hasData && isUserJoined.data!) ? Icons.arrow_circle_left_outlined : Icons.login,
                                            color: Colors.white,
                                          ),
                                    label: (isUserJoined.connectionState == ConnectionState.waiting)
                                        ? CircularProgressIndicator(color: Colors.white)
                                        : Txt(isUserJoined.hasData && (isUserJoined.data!) ? "Leave" : "Join", textColor: Colors.white));
                              });
                        });
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          StreamBuilder<OrganizerModel>(
                              stream: FireServices.instance.fetchSingleOrganizer(id: event.orgId!),
                              builder: (context, org) {
                                return ListTile(
                                  title: Txt(org.data?.organizer ?? "--", fontsize: 2.t, fontweight: FontWeight.w500),
                                  subtitle: Txt(org.data?.organization ?? "--", fontsize: 1.5.t, fontweight: FontWeight.w400),
                                  leading: AspectRatio(
                                      aspectRatio: 1,
                                      child: (org.data != null && org.data?.image != null && org.data?.image?.trim() != "")
                                          ? org.data!.image!.startsWith('http')
                                              ? ClipOval(
                                              ...
                                              : ClipOval(
                                                  child: Container(
                                                    decoration:
                                                        BoxDecoration(borderRadius: BorderRadius.circular(15), image: DecorationImage(image: AssetImage(Images.userPlaceholder), fit: BoxFit.cover)),
                                                  ),
                                                )
                                          : ClipOval(
                                              child: Container(
