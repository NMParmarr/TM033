...
class _OrgProfileScreenState extends State<OrgProfileScreen> {
  ...
  bool validateNewUserTextFields() {
    if (_newUserFullNameCtr?.text.toString().trim() == "") {
      ...
      return false;
      //
    } else if (!Utils.isValidMobile(mobile: _newUserMobileCtr!.text.toString().trim())) {
      ...
      return false;
      //
    } else if (!Utils.isValidLengthPassword(password: _newUserPassCtr!.text.toString().trim())) {
          ...
          builder: (context, orgId) {
            if (orgId.hasData) {
              return StreamBuilder<OrganizerModel>(
                  stream: FireServices.instance.fetchSingleOrganizer(id: orgId.data!),
                  builder: (context, currentOrgSnap) {
                    if (currentOrgSnap.hasData) {
                      return _contentWidget(context, org: currentOrgSnap.data, orgId: orgId.data!);
  ...
  Widget _contentWidget(BuildContext context, {required OrganizerModel? org, required String orgId}) {
        ...
        Padding(
          ...
          child: Row(
            children: [
              Hero(
                  tag: "orgprofile",
                  child: (org != null && org.image != null && org.image?.trim() != "")
              ...
              Column(
                ...
                children: [Txt(org?.organizer ?? "--", textColor: Colors.black, fontsize: 2.4.t), Txt(org?.email?.toLowerCase() ?? "--", textColor: Colors.black, fontsize: 1.7.t)],
              )
            ],
          ),
        ),
            ...
            children: [
              Expanded(
                  child: ElevatedButton.icon(
                      ...
                      onPressed: () {
                        Navigator.pushNamed(context, Routes.editOrgProfile, arguments: {'org': org});
                      },
          ...
          children: [
            Scaffold(
              body: SingleChildScrollView(
                ...
                child: StreamBuilder<List<UserModel>>(
                    stream: FireServices.instance.fetchUsersByOrdId(orgId: orgId),
                    builder: (context, usersSnap) {
                      if (usersSnap.hasData) {
                        if (usersSnap.data?.length == 0) {
                          return Utils.noDataFoundWidget(msg: "No Users..!", alignment: Alignment.topCenter);
                        } else {
                          return UsersList(usersList: usersSnap.data ?? []);
                        }
            ...
            Padding(
              ...
              child: ListView(
                children: [
                  VGap(1.3.h),
                  Txt("Organization Name", fontsize: 2.5.t, fontweight: FontWeight.w600, textColor: Colors.black),
                  VGap(0.3.h),
                  Txt(org?.organization ?? "--", fontsize: 2.t, textColor: Colors.black),
                  VGap(1.3.h),
                  Txt("Mobile", fontsize: 2.5.t, fontweight: FontWeight.w600, textColor: Colors.black),
                  VGap(0.3.h),
                  Txt(org?.mobile != null ? "+91 ${org!.mobile}" : "--", fontsize: 2.t, textColor: Colors.black),
                  VGap(1.3.h),
                  Txt("About", fontsize: 2.5.t, fontweight: FontWeight.w600, textColor: Colors.black),
                  VGap(0.3.h),
                  Txt(
                    org?.about ?? "--------\n-------------",
                    fontsize: 2.t,
                  ...
                  Builder(builder: (context) {
                    String formattedJoinedDate = DateFormat('dd MMM yyyy').format(DateTime.parse(org!.joinDate!));
                    return Txt(
                      formattedJoinedDate,
                      fontsize: 2.t,
                      textColor: Colors.black,
  ...
  Future<dynamic> _addUserBottomSheet(BuildContext context, {required String orgId}) {
    ...
    return showModalBottomSheet(
      ...
      builder: (context) {
        return Padding(
            padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, left: 3.w, right: 3.w, top: 2.w),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                ...
                CustomTextField(ctr: _newUserFullNameCtr!, hintText: "Enter full name of new user", capitalization: TextCapitalization.words),
                VGap(1.5.h),
                ...
                VGap(1.5.h),
                Txt("Password", textColor: Colors.black, fontsize: 2.t, fontweight: FontWeight.w500),
                CustomTextField(
                  ctr: _newUserPassCtr!,
                      ...
                      return ElevatedButton.icon(
                          ...
                          onPressed: () async {
                            ...
                            final bool res = await provider.addNewUser(
                                orgId: orgId,
                                fullName: _newUserFullNameCtr!.text.toString().trim(),
                            ...
                            if (res) {
                              Navigator.pop(context);
                              clearTextControllers();
                              showFlushbar(context, "New User Added Succeesfully..!");
                            }
                          },
                          icon: provider.newUserLoading ? SizedBox() : Icon(Icons.person_add_alt_1_sharp, color: Colors.white),
                          label: provider.newUserLoading ? CircularProgressIndicator(color: Colors.white) : Txt("Add", textColor: Colors.white));
                    })),
                  ],
                ),
                VGap(2.h)
